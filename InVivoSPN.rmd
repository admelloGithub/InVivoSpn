...
bac <- ggplot(inp, aes(x=inp$Sample, y=log10(inp$Spn), color = inp$Sample, fill = inp$Sample)) + geom_histogram(stat = "identity", show.legend = FALSE, alpha =0.6)  + #coord_cartesian(ylim=c(0,1000000)) + 
...
host <- ggplot(inp, aes(x=inp$Sample, y=log10(inp$Mouse), color = inp$Sample, fill = inp$Sample)) + geom_histogram(stat = "identity", show.legend = FALSE, alpha =0.6) + scale_color_manual(values = colors) + scale_fill_manual(values = colors) + geom_hline(yintercept = log10(150000)) +#coord_cartesian(ylim=c(0,1000000)) + 
...
x6a10 <- rownames(upset[upset$`6A10_Inf_Naso_vs_Uninf_Naso`==1,])
t4 <- rownames(upset[upset$TIGR4_Inf_Naso_vs_Uninf_Naso==1,])
d39 <- rownames(upset[upset$D39_Inf_Naso_vs_Uninf_Naso==1,])

colsep <- get_heatmap_separators(as.character(design$condition))
hmcol <- colorRampPalette(c("navyblue","white","firebrick3"))(40)
png(filename = paste0(output_dir,"naso6a10.png"),width = 1720, height = 1720, units = "px")
heatmap.2(t(scale(t(counts.vsd[x6a10,c(31:32,37:38)]))),
          keysize=1,
          margins = c(24,10),
          breaks = seq(-2, 2, 0.1), 
          col=hmcol,
          ...
          Rowv = T,
          Colv = F,
          ColSideColors=as.vector(grp.col)[c(31:32,37:38)],
          rowsep = F,
          dendrogram = "none",
...
png(filename = paste0(output_dir,"nasotigr4.png"),width = 1720, height = 1720, units = "px")
heatmap.2(t(scale(t(counts.vsd[t4,c(35:38)]))),
          keysize=1,
          margins = c(24,10),
          breaks = seq(-2, 2, 0.1), 
          col=hmcol,
...
png(filename = paste0(output_dir,"nasod39.png"),width = 1720, height = 1720, units = "px")
heatmap.2(t(scale(t(counts.vsd[d39,c(33:34,37:38)]))),
          keysize=1,
          margins = c(24,10),
          breaks = seq(-2, 2, 0.1), 
          col=hmcol,
...
n1 <- rasterGrob(readPNG(paste0(output_dir,"/nasod39.png"), native = FALSE),interpolate = FALSE)
n2 <- rasterGrob(readPNG(paste0(output_dir,"/nasotigr4.png"), native = FALSE),interpolate = FALSE)
n3 <- rasterGrob(readPNG(paste0(output_dir,"/naso6a10.png"), native = FALSE),interpolate = FALSE)


pdf(paste0(output_dir,'/suppfig3.pdf'), paper = "a4r", width = 0, height = 0)  ## Device with dimensions of A4 paper
grid.arrange(arrangeGrob(n1,n2,n3,nrow = 1, ncol =3),nrow = 1, ncol =1)
dev.off()
...
colnames(ceg.upset) = c("Blood","Lung","Nasopharynx","Heart","Kidney")
ceg.upset[is.na(ceg.upset)]<-0
ceg.upset.plot <- upset(ceg.upset, queries = list(list(query = intersects, params = list("Blood","Lung","Nasopharynx","Heart","Kidney"), color = "black", active = T)),sets = colnames(ceg.upset), sets.bar.color = "orange", point.size = 4,text.scale = 4, main.bar.color = "grey50",
...
png(filename = paste0(output_dir,"ceg52.png"),width = 1720, height = 1780, units = "px")
...
#Host stuff
bhm.path <- paste0(input_dir,"/BloodHM.txt") 
hhm.path <- paste0(input_dir,"/HeartHM.txt") 
khm.path <- paste0(input_dir,"/KidneyHM.txt")  
lhm.path <- paste0(input_dir,"/LungHM.txt")  
ahm.path <- paste0(input_dir,"/InforgsCanPathHM.txt")  
...
colnames(bhm) <- gsub("X6A10", "6A10", colnames(bhm))
colnames(hhm) <- gsub("_FDR.*","",colnames(hhm))
colnames(hhm) <- gsub("X6A10", "6A10", colnames(hhm))
colnames(khm) <- gsub("_FDR.*","",colnames(khm))
colnames(khm) <- gsub("X6A10", "6A10", colnames(khm))
colnames(lhm) <- gsub("_FDR.*","",colnames(lhm))
colnames(lhm) <- gsub("X6A10", "6A10", colnames(lhm))
colnames(ahm) <- gsub("_FDR.*","",colnames(ahm))
colnames(ahm) <- gsub("X6A10", "6A10", colnames(ahm))
...
bhm <- bhm[abs(rowMeans2(bhm))>=2,]
ord <- hclust(d = dist(bhm))$order
ord2 <- hclust(d = dist(t(bhm)))$order
bhm <- bhm[rev(ord),ord2]
bhm <- melt(bhm)
colnames(bhm) = c("y","x","z")
png(filename = paste0(output_dir,"bipa.png"),width = 1500, height = 2000, units = "px")
#bipa <- 
ggplot(bhm) + 
  geom_tile(aes(x,y,fill=z))+
  scale_fill_gradientn(colours=hmcol)+
  theme(axis.text.x=element_text(angle=-90,vjust=.2, hjust=0, size = 30), axis.text.y =element_text(size = 30), title = element_text(size = 35),legend.key.size = unit(1.5,"cm"),legend.text = element_text(size = 25), plot.title = element_text(hjust = 0.5))+
...
hhm <- as.matrix(hhm)
hhm <- hhm[abs(rowSums(hhm))>=2,] 
hhm <- rev(sort(hhm))
hhm <- data.frame(TIGR4_Inf_Heart_vs_Uninf_Heart = as.vector(hhm), row.names = names(hhm))
hdf <- data.frame(P = rownames(hhm), S= "TIGR4_Inf_Heart_vs_Uninf_Heart", value = hhm$TIGR4_Inf_Heart_vs_Uninf_Heart)
colnames(hdf) = c("y","x","z")
png(filename = paste0(output_dir,"hipa.png"),width = 1500, height = 2000, units = "px")
#hipa <- 
ggplot(hdf) + 
  geom_tile(aes(x=x,y=reorder(y, z),fill=z))+
  scale_fill_gradientn(colours=hmcol)+
  theme(axis.text.x=element_text(angle=-90,vjust=.2, hjust=0, size = 30), axis.text.y =element_text(size = 30), title = element_text(size = 35),legend.key.size = unit(1.5,"cm"),legend.text = element_text(size = 25), plot.title = element_text(hjust = 0.5))+
...
khm <- khm[abs(rowMeans2(khm))>=2,]
ord <- hclust(d = dist(khm))$order
ord2 <- hclust(d = dist(t(khm)))$order
khm <- khm[rev(ord),ord2]
khm <- melt(khm)
colnames(khm) = c("y","x","z")
png(filename = paste0(output_dir,"kipa.png"),width = 1500, height = 2000, units = "px")
#kipa <- 
ggplot(khm) + 
  geom_tile(aes(x,y,fill=z))+
  scale_fill_gradientn(colours=hmcol)+
...
lhm <- lhm[abs(rowMeans2(lhm))>=2,]
ord <- hclust(d = dist(lhm))$order
ord2 <- hclust(d = dist(t(lhm)))$order
lhm <- lhm[rev(ord),ord2]
lhm <- melt(lhm)
colnames(lhm) = c("y","x","z")
png(filename = paste0(output_dir,"lipa.png"),width = 1500, height = 2000, units = "px")
#lipa <- 
  ggplot(lhm) + 
  geom_tile(aes(x,y,fill=z))+
  scale_fill_gradientn(colours=hmcol)+
...
ahm <- ahm[abs(rowMeans2(ahm))>=2,]
ord <- hclust(d = dist(ahm))$order
ord2 <- hclust(d = dist(t(ahm)))$order
ahm <- ahm[rev(ord),ord2]
ahm <- melt(ahm)
colnames(ahm) = c("y","x","z")
png(filename = paste0(output_dir,"aipa.png"),width = 1500, height = 2000, units = "px")
#aipa <- 
  ggplot(ahm) + 
  geom_tile(aes(x,y,fill=z))+
  scale_fill_gradientn(colours=hmcol)+
...
#All DE genes (strain specifics TIGR4)

```{r}


counts.path <- paste0(input_dir,"/TIGR4_counts.txt")
groups.path <- paste0(input_dir,"/TIGR4_design.txt")  
comparisons <- c("TIGR4_Blood_vs_TIGR4_Naso","TIGR4_Heart_vs_TIGR4_Naso","TIGR4_Lung_vs_TIGR4_Naso","TIGR4_Kidney_vs_TIGR4_Naso")
...
for (i in contrast.list$Contrast) {
	pair <- as.data.frame(strsplit(as.character(i),'_vs_'))
	DE.genes <- as.data.frame(results(dds, contrast=c("condition",as.character(pair[1,]),as.character(pair[2,])), independentFiltering = FALSE, alpha=0.05))
	DE.genes$Significant = as.factor(DE.genes$padj < 0.05)# & abs(DE.genes$log2FoldChange) > 1)
	write.table(DE.genes, file = paste0(output_dir,"/",i,"_unfiltered_degenes.txt"), append = F, row.names = T,sep = "\t")
	genes <- as.data.frame(c(rep_len(1,length.out = length(row.names(DE.genes)))),row.names = rownames(DE.genes) )
	upset <- merge.all(upset,genes)
}
...
#All DE genes (strain specifics 6A10)

```{r}


counts.path <- paste0(input_dir,"/6A10_counts.txt")
groups.path <- paste0(input_dir,"/6A10_design.txt")  
comparisons <- c("6A10_Blood_vs_6A10_Naso","6A10_Lung_vs_6A10_Naso")
